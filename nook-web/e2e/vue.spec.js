...
test.describe('inactive card interactions', () => {
  test.use({ viewport: { width: 834, height: 1112 }, hasTouch: true })

  test('uses tablet overlays and mobile sheets for ratings, reminders, start and resume', async ({ page }, testInfo) => {
    await mockSignedIn(page)
    const shows = [
      { _id: '507f1f77bcf86cd799439091', title: '待启程', status: 'wish', airedEpisodes: 3, watchedEpisodes: 0, totalEpisodes: 28, releaseDate: '2026-09-01' },
      { _id: '507f1f77bcf86cd799439092', title: '旧时光', status: 'watched', airedEpisodes: 28, watchedEpisodes: 28, totalEpisodes: 28, completedAt: '2026-08-12', personalRating: null },
      { _id: '507f1f77bcf86cd799439093', title: '未完旅途', status: 'dropped', airedEpisodes: 20, watchedEpisodes: 12, totalEpisodes: 28 },
      { _id: '507f1f77bcf86cd799439094', title: '明日来信', status: 'wish', airedEpisodes: 0, watchedEpisodes: 0, totalEpisodes: 12, releaseDate: '2026-10-01' },
    ].map(show => ({ category: 'tv', network: '优酷', updateFrequency: 'weekly', ...show }))
    await page.route('**/api/shows/calendar', route => fulfillJson(route, shows))
    await page.route('**/api/tmdb/trending', route => fulfillJson(route, []))
    await page.route(/\/api\/shows(?:\?.*)?$/, route => fulfillJson(route, { items: shows, pagination: { page: 1, total: 4, hasMore: false }, facets: { allCount: 4, statusCounts: { wish: 2, watched: 1, dropped: 1 }, categoryCounts: {}, networks: [] } }))
    let failRating = true
    await page.route(/\/api\/shows\/507f1f77bcf86cd79943909[1-4]$/, route => {
      const show = shows.find(show => route.request().url().endsWith(show._id))
      const patch = route.request().postDataJSON()
      if ('personalRating' in patch && failRating) return fulfillJson(route, { error: '暂时无法保存评分' }, 503)
      Object.assign(show, patch)
      return fulfillJson(route, show)
    })
    await page.goto('/home/tv-shows')
    const inactive = page.locator('.inactive-card')
    await expect(inactive).toHaveCount(4)
    await expect(inactive.locator('.progress-control')).toHaveCount(0)
    const wish = inactive.filter({ hasText: '待启程' })
    await expect(wish.locator('.poster-overlay')).toBeHidden()
    await wish.getByRole('button', { name: '待启程 状态操作' }).tap()
    await expect(wish.locator('.poster-overlay')).toBeVisible()
    await page.screenshot({ path: testInfo.outputPath('inactive-tablet.png') })
    await wish.getByRole('button', { name: '开始追剧', exact: true }).tap()
    await expect(inactive).toHaveCount(3)
    expect(shows[0].watchedEpisodes).toBe(0)
    expect(shows[0].trackingStarted).toBe(true)
    await page.setViewportSize({ width: 390, height: 844 })
    await page.getByRole('button', { name: '旧时光 状态操作' }).tap()
    const sheet = page.getByRole('dialog', { name: '旧时光 操作' })
    await sheet.getByLabel('个人评分', { exact: true }).selectOption('9')
    await expect(sheet.getByRole('alert')).toContainText('暂时无法保存评分')
    failRating = false
    await sheet.getByLabel('个人评分', { exact: true }).selectOption('9')
    await expect(sheet.getByRole('status')).toContainText('已保存')
    expect(shows[1].personalRating).toBe(9)
    await page.screenshot({ path: testInfo.outputPath('inactive-mobile.png') })
    await sheet.getByRole('button', { name: '重温 / 重新在看' }).tap()
    await expect(sheet).toContainText('保留原观影日志')
    await sheet.getByRole('button', { name: '确认开始重温' }).tap()
    await expect(sheet).toHaveCount(0)
    expect(shows[1].completedAt).toBe('2026-08-12')
    expect(shows[1].watchedEpisodes).toBe(0)
    await page.getByRole('button', { name: '未完旅途 状态操作' }).tap()
    const dropped = page.getByRole('dialog', { name: '未完旅途 操作' })
    await expect(dropped).toContainText('停留在 第 12 集')
    await dropped.getByRole('button', { name: '恢复追剧' }).tap()
    await expect(dropped).toHaveCount(0)
    expect(shows[2].watchedEpisodes).toBe(12)
    await page.getByRole('button', { name: '明日来信 状态操作' }).tap()
    const premiere = page.getByRole('dialog', { name: '明日来信 操作' })
    await premiere.getByRole('button', { name: '开播提醒', exact: true }).tap()
    await expect(premiere.getByRole('button', { name: '取消开播提醒' })).toBeVisible()
    await premiere.getByRole('button', { name: '关闭作品操作' }).tap()
    shows[3].airedEpisodes = 1
    await page.reload()
    await expect(page.locator('.premiere-notice')).toContainText('明日来信')
  })
})

test('desktop inactive cards reveal actions on hover and remain keyboard accessible', async ({ page }, testInfo) => {
  await page.setViewportSize({ width: 1440, height: 1000 })
  await mockSignedIn(page)
  const shows = ['wish', 'watched', 'dropped'].map((status, index) => ({ _id: `507f1f77bcf86cd79943907${index}`, title: ['远方来信', '夏日终曲', '旅途未央'][index], category: 'tv', status, watchedEpisodes: [0, 28, 12][index], airedEpisodes: 28, totalEpisodes: 28, network: '腾讯视频', personalRating: status === 'watched' ? 9 : null }))
  await page.route('**/api/shows/calendar', route => fulfillJson(route, shows))
  await page.route('**/api/tmdb/trending', route => fulfillJson(route, []))
  await page.route(/\/api\/shows(?:\?.*)?$/, route => fulfillJson(route, { items: shows, pagination: { page: 1, total: 3, hasMore: false }, facets: { allCount: 3, statusCounts: {}, categoryCounts: {}, networks: [] } }))
  await page.goto('/home/tv-shows')
  const card = page.locator('.inactive-card').filter({ hasText: '夏日终曲' })
  await expect(card.locator('.poster-overlay')).toBeHidden()
  await card.locator('.poster-trigger').hover()
  await expect(card.locator('.poster-overlay')).toBeVisible()
  await expect(card.getByRole('button', { name: '重温 / 重新在看' })).toBeVisible()
  await expect(card.locator('.poster-overlay')).toHaveCSS('opacity', '1')
  await page.screenshot({ path: testInfo.outputPath('inactive-desktop.png') })
  await page.getByRole('heading', { name: '追剧记录', exact: true }).hover()
  await card.locator('.poster-trigger').focus()
  await expect(card.locator('.poster-overlay')).toBeVisible()
  await page.keyboard.press('Escape')
  await expect(card.locator('.card-title')).toBeFocused()
  await expect(card.locator('.poster-overlay')).toBeHidden()
})
